...
dqt = [-1/2*sin(theta/2)*dtheta, 1/2*cos(theta/2)*dtheta*quatrotate(q.', [0 0 1])+sin(theta/2)*(dR*[0 0 1].')'].';
a = -1/2 * (sin(theta/2)*ddtheta + 1/2 * cos(theta/2)*(dtheta)^2);
b = 1/2 * ((cos(theta/2)*ddtheta - 1/2 * sin(theta/2)*(dtheta)^2)*quatrotate(q.', [0 0 1]) + 1/2 * cos(theta/2) * dtheta * (dR*[0 0 1].').');
