...
qt = [cos(theta/2) sin(theta/2)*O_z_B].';
% Sym variable
ddq0 = sym('ddq0','real');
ddq1 = sym('ddq1','real');
ddq2 = sym('ddq2','real');
...
ddqt = [a b+c].';
% Force equation
eqn1 = (m_B + m_C)*ddp + m_C * ddR * r_BC == F_GC + F_GB + quatrotate(q.',F_T.').';
% Torque equation
Lterm1 = 2*qMatrix(I_B)*(quatmultiply(ddq.',quatconj(q.'))).';
Lterm2 = 2*qMatrix(I_C)*quatmultiply(quatmultiply(qt.',ddq.'), quatconj(quatmultiply(qt.',q.'))).';
...
Rterm2 = 2*qMatrix(I_C)*(qSquare(quatmultiply(quatmultiply(qt.', dq.') + quatmultiply(dqt.', q.'), quatconj(quatmultiply(qt.', q.'))))).';
...
eqn4 = qt(1)*ddqt(1)+qt(2)*ddqt(2)+qt(3)*ddqt(3)+qt(4)*ddqt(4)+dqt(1)^2 +dqt(2)^2 +dqt(3)^2 +dqt(4)^2 == 0;
sol = solve([eqn1, eqn2, eqn3, eqn4], [ddpx, ddpy, ddpz, ddq0, ddq1, ddq2, ddq3, ddtheta]);
end
